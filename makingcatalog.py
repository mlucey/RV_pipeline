...
#path to the rvcorrect, halphalegend and rv.dat file once in the obsrun directory 
rvpath = '/results/'
...
for i in obsrun:
    ...
    ohalpharas = np.loadtxt(inpath+i+rvpath+'Halphalegend.txt', usecols = (1,),dtype=str)
...
with open(inpath+'halphaonly_'+cluster+'_catalog.csv','w') as f:
    writer = csv.writer(f)
    writer.writerow(['RA','DEC','r','r-J','obsrun','image','ap','hjd','rv','err','hght'])
